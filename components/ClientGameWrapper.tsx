...
type LeaderboardEntry = { id: number; username: string; score: number; };
...
export default function ClientGameWrapper() {
  ...
  const handleSubmit = async (formData: FormData) => {
    setIsSaving(true);
    // Minimum 3s wait to ensure they see the interstitial ad overlay
    await Promise.all([
  ...
  return (
    <>
      {finalScore === null ? (
        ...
        /* GAME OVER SCREEN */
        <div className="flex flex-col items-center w-full max-w-lg mx-auto bg-white p-6 rounded-xl shadow-lg border-2 border-green-500 mt-6">
          ...
          {/* AD SLOT 1: Shown while user is typing name */}
          <div className="w-full flex justify-center mb-4 bg-gray-50 border p-2">
            ...
            {/* Rectangle Ad for High Visibility */}
          ...
          <form action={handleSubmit} className="flex flex-col gap-3 w-full">
            ...
            <input type="text" name="username" placeholder="Enter Your Name" required className="p-3 border border-gray-300 rounded-lg w-full" maxLength={15} />
      ...
      {isSaving && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 modal-overlay">
          <div className="bg-white p-6 rounded-xl max-w-md w-full flex flex-col items-center text-center m-4">
            ...
            {/* AD SLOT 2: "Video" Ad during save */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 modal-overlay backdrop-blur-sm">
            <div className="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden m-4 flex flex-col max-h-[80vh]">
                <div className="p-4 bg-green-600 text-white flex justify-between items-center">
                    ...
                    <button onClick={() => setShowLeaderboard(false)} className="text-2xl">&times;</button>
                </div>
                <div className="p-4 overflow-y-auto flex-1">
                    {loadingLeaderboard ? <div className="flex justify-center p-8"><div className="loader"></div></div> : (
                        <table className="w-full text-left">
                            <thead className="bg-gray-50 text-gray-600 text-sm"><tr><th className="p-2">#</th><th className="p-2">Name</th><th className="p-2 text-right">Score</th></tr></thead>
                            <tbody className="divide-y">{leaderboardData.map((e, i) => (<tr key={e.id}><td className="p-3">#{i+1}</td><td className="p-3">{e.username}</td><td className="p-3 text-right font-bold text-green-700">{e.score}</td></tr>))}</tbody>
                        </table>
                    )}
                </div>
            </div>
